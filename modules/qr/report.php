...
if (isset($_REQUEST["course"])) {
	$results = Core::$systemDB->executeQuery("select studentNumber, name, campus, type, description "
...
foreach ($results as $result) {
	...
	echo ("{$result['studentNumber']}" . $sep . "{$student_name}" . $sep . "{$result['campus']}" . $sep . "participated in {$result['type']}" . $sep . $v . "{$result['description']}\n");
}
	//pg_close($connection);
