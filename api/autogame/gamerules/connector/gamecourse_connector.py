...
def calculate_xp(course, target):
    ...
    query = "SELECT id, max(minXP) from level where minXP <= %s and course = %s group by id order by number desc limit 1;"
...
def award_badge(target, badge, lvl, contributions=None, info=None):
    ...
    # get badge info
    query = "SELECT number, badge, reward from badge_level left join badge on badge.id = badge_level.badge where badge.course = %s and badge.name = %s order by number;"
...
def get_campus(target):
    ...
    course = config.course
    query = "select major from course_user left join user on course_user.id=user.id where course = %s and course_user.id = %s;"
