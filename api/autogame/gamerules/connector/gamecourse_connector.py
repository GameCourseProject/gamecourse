...
def get_student_numbers(course):
    ...
    query = "SELECT id, studentNumber FROM user;"
...
def calculate_teams_xp(course, target):
    ...
    query = "SELECT id, max(minXP) from level where minXP <= \"" +  str(team_xp) + "\" and course = \"" + str(course) + "\" group by id order by minXP desc limit 1;"
...
def calculate_xp(course, target):
    ...
    query = "SELECT maxExtraCredit from badges_config where course = \"" + str(course) + "\";"
    result = db.course_data_broker(course, query)
    if not result:
        cursor.execute(query)
    ...
    query = "SELECT maxExtraCredit from streaks_config where course = \"" + str(course) + "\";"
    result = db.course_data_broker(course, query)
    if not result:
        cursor.execute(query)
    ...
    query = "SELECT id, max(minXP) from level where minXP <= \"" +  str(total_xp) + "\" and course = \"" + str(course) + "\" group by id order by minXP desc limit 1;"
    result = db.course_data_broker(course, query)
    if not result:
        cursor.execute(query)
        level_table = cursor.fetchall()
...
def award_badge(target, badge, lvl, contributions=None, info=None):
    ...
    # get badge info
    query = "SELECT badge_level.number, badge_level.badge, badge_level.reward, badge.isActive from badge_level left join badge on badge.id = badge_level.badge where badge.course = \"" + course + "\" and badge.name = \"" + badge + "\" order by number;"
    result = db.course_data_broker(course, query)
    if not result:
        cursor.execute(query)
...
def award_skill(target, skill, rating, contributions=None, use_wildcard=False, wildcard_tier=None):
    ...
    if not config.test_mode:
        ...
        if use_wildcard != False and wildcard_tier != None:
            ...
            query = "select t.id from skill_tier t left join skill_tree s on t.skillTree=s.id where t.name = \"" + wildcard_tier + "\" and course = \"" + course + "\";"
            result = db.course_data_broker(course, query)
            if not result:
                cursor.execute(query)
        ...
        query = "SELECT s.id, reward, s.tier, s.isActive FROM skill s join skill_tier on s.tier=skill_tier.id join skill_tree t on t.id=skill_tier.skillTree where s.name = \""+ skill +"\" and course = \""+ course +"\";"
        result = db.course_data_broker(course, query)
        if not result:
            cursor.execute(query)
        ...
        # first skill awarded cost 0 tokens
            ...
            if use_wildcard != False and wildcard_tier != None:
                ...
                query = "INSERT INTO award_wildcard (user, course, award) VALUES (%s,%s,%s);"
                cursor.execute(query, (target, course, award_id))
                connect.commit()
