...
def Squire(logs, student):
    views,values, total = FilterGrades(logs,student,"Tutorials",max_repeats=None)
...
##              "PCM\tGabriel Barata\tParticipation\tTheoretical Challenge #4\t2\txpto.html"]:
      if len(l.strip())>0:
          ...
          try:
              ...
              ignored_lines+=1
              continue
          #name=str(name, "latin1")  # because that is what we get from the logs now
          name=str(name)  # because that is what we get from the logs now
