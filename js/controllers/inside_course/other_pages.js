...
app.controller("CourseUsersss", function($scope, $stateParams, $element, $smartboards, $compile, $parse) {
    ...
    $scope.createUser = function() {
        ...
        doubledetails.append($('<div class="details half"><div class="container" ><input type="text" class="form__input" id="studentNumber" placeholder="Student Number *" ng-model="newUser.userStudentNumber"/><label for="studentNumber" class="form__label">Number</label></div></div>'));
        doubledetails.append($('<div class="details half right"><div class="container" ><input type="text" class="form__input" id="major" placeholder="Major *" ng-model="newUser.userMajor"/><label for="major" class="form__label">Major</label></div></div>'));
        ...
        $scope.isReadyToSubmit = function() {
            ...
            //validate inputs
            if (isValid($scope.newUser.userName) && isValid($scope.newUser.userStudentNumber) && isValid($scope.newUser.userEmail) && isValid($scope.newUser.userUsername) && isValid($scope.newUser.userAuthService) && isValid($scope.newUser.userMajor) && $scope.newUser.userRoles.length != 0) {
                return true;
            } else {
                return false;
            }
    ...
    updateRolesAddSection = function($scope, box, allRoles, userRoles) {
        ...
        $("#add_roles").remove();
        editbox = $(box);
        remianingRoles = allRoles.filter(
            (el) => !userRoles.includes(el));
    ...
    $scope.modifyUser = function(user) {
        ...
        $scope.isReadyToEdit = function() {
            ...
            //validate inputs
            if (isValid($scope.editUser.userName) && isValid($scope.editUser.userEmail) && isValid($scope.editUser.userStudentNumber) && isValid($scope.editUser.userMajor) && isValid($scope.editUser.userUsername) && isValid($scope.editUser.userAuthService) && $scope.editUser.userRoles.length != 0) {
                return true;
            } else {
                return false;
            }
    ...
    $smartboards.request("course", "courseRoles", { course: $scope.course }, function(data, err) {
        ...
        editdoubledetails.append($('<div class="details half"><div class="container" ><input type="text" class="form__input" id="studentNumber" placeholder="Student Number *" ng-model="editUser.userStudentNumber"/><label for="studentNumber" class="form__label">Number</label></div></div>'));
        editdoubledetails.append($('<div class="details half right"><div class="container" ><input type="text" class="form__input" id="major" placeholder="Major *" ng-model="editUser.userMajor"/><label for="major" class="form__label">Major</label></div></div>'));
