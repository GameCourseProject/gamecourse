...
;(function($, window, document, undefined)
{
    ...
    Plugin.prototype = {
        ...
        initializeItem: function(el) {
            ...
            list.setParent(el);
            list.createAdd(el);
            list.createRemove(el);
            if (el.hasClass(list.options.collapsedClass))
                list.collapseItem(el);
        },

        createItem: function(li, text, data) {
            var list = this;
            var item = $('<' + list.options.itemNodeName + '>', {'class': list.options.itemClass});
            if (data) {
            ...
            list.createAdd(item);
            list.createRemove(item);
            return item;
        },

        createRootItem: function() {
            var list = this;
            return function(text, data, itemCreatedCallback) {
                var item = list.createItem(list.el.children('.' + list.options.listClass), text, data);
                if (itemCreatedCallback) {
                    itemCreatedCallback(item);
                }
            };
        },

        createAdd: function(item) {
            var list = this;
        ...
        createRemove: function(item) {
            ...
            imgRemove.click(function(e) {
                ...
                root.trigger('removeitem', [data, cancel]);

                if (shouldCancel)
                    return;

                if (myList.children().length == 1 && myList.parent().hasClass(list.options.itemClass)) {
                    list.unsetParent(myList.parent());
                    myList.remove();
                } else
                    item.remove();
        ...
        {
            if (li.parent().children('.' + this.options.itemClass + ' button').length == 0)
                li.parent().removeClass(this.options.hasChildrenClass);
            li.removeClass(this.options.collapsedClass);
        ...
        dragStart: function(e)
        {
            ...
            if (dragItem.children(this.options.listNodeName).length)
                this.dragEl.addClass(this.options.hasChildrenClass);
        ...
        dragStop: function(e)
        {
            ...
            if (el.children(this.options.listNodeName).length)
                this.setParent(el);
        ...
        dragMove: function(e)
        {
            ...
            if (mouse.dirAx && mouse.distAxX >= opt.threshold) {
                ...
                if (mouse.distX < 0) {
                    ...
                    if (!next.length && !this.placeEl.closest(opt.itemNodeName).parent().hasClass(opt.noDropClass)) {
            ...
            if (!mouse.dirAx || isNewRoot || isEmpty) {
                ...
                if (isEmpty) {
                    ...
                    if(!this.pointEl.parent().hasClass(opt.noDropClass))
