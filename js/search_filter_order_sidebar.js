function createSidebar(optionsFilter, optionsOrder) {
  ...
  search = $(
    "<div class='search'> <input type='text' id='seach_input' placeholder='Search..' name='search' ng-change='reduceList()' ng-model='search' ><button class='magnifying-glass' id='search-btn' ng-click='reduceList()'></button>  </div>"
  );
      ...
      filter.append(
        $(
          "<label class='container'>" +
            ...
            optionsFilter[index] +
            "' ng-change='reduceList()' ng-model='filter" +
            optionsFilter[index] +
  ...
  if (optionsOrder.length > 0) {
    oderby = $(
      "<div class='order-by'> <div class='title'>Order by</div></div>"
    );
        ...
        oderby.append(
          $(
            "<label class='container'>" +
              ...
              transformNameToId(optionsOrder[index]) +
              "' ng-click='orderList()'><span class='checkmark'></span>   </label>"
          )
        );
      } else {
        oderby.append(
    ...
    oderby.append(
      $(
        '<div class="sort"><div class="triangle icon" id="triangle-up" ng-click="sortUp(); orderList();"></div><div class="triangle icon checked" id="triangle-down" ng-click="sortDown(); orderList();"></div></div>'
      )
    );
...
function orberByNickname(a, b) {
  if (a.nickname == null || a.nickname > b.nickname) {
    return 1;
  }
  if (b.nickname == null || a.nickname < b.nickname) {
    return -1;
  }
  return 0;
}
function orberByStudentNumber(a, b) {
  if (
    ...
    parseInt(a.studentNumber) > parseInt(b.studentNumber)
  ) {
    return 1;
  }
  if (
    b.studentNumber == null ||
    parseInt(a.studentNumber) < parseInt(b.studentNumber)
  ) {
    return -1;
  }
...
function orberByLastLgin(a, b) {
  ...
  if (astate > bstate) {
    ...
    return -1;
  } else if (astate == 0 || astate == 6) {
    return 0;
  } else {
    //evaluates the number part for equal states
    if (parseInt(a.lastLogin) > parseInt(b.lastLogin)) {
      return 1;
    }
    if (parseInt(a.lastLogin) < parseInt(b.lastLogin)) {
      return -1;
...
function orberByNCourses(a, b) {
  if (a.ncourses == null || parseInt(a.ncourses) > parseInt(b.ncourses)) {
    return 1;
  }
  if (b.ncourses == null || parseInt(a.ncourses) < parseInt(b.ncourses)) {
    return -1;
  }
  return 0;
}
function orberByShort(a, b) {
  if (a.short == null || a.short > b.short) {
    return 1;
  }
  if (b.short == null || a.short < b.short) {
    return -1;
  }
  return 0;
}
function orberByYear(a, b) {
  ...
  if (b.year == null || a.year < b.year) {
    return -1;
  }
  return 0;
}
function orberByNStudents(a, b) {
  if (a.nstudents == null || parseInt(a.nstudents) > parseInt(b.nstudents)) {
    return 1;
  }
  if (b.nstudents == null || parseInt(a.nstudents) < parseInt(b.nstudents)) {
    return -1;
  }
  return 0;
}
