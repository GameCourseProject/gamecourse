...
import {EventType} from "../../../../../../../_domain/views/events/event-type";
...
export class ViewsEditorComponent implements OnInit {
  ...
  partToAdd: string;
  fakeIDMin: number = 0; // need negative fake IDs for building-blocks that are being added and not in DB yet
  ...
  viewsDeleted: number[] = []; // viewIds of building-blocks that were deleted; check if need to be deleted from database
  ...
  previewView() {
    ...
    //   .pipe( finalize(() => this.loading = false) )
    //   .subscribe(view => this.viewToPreview = view)
  }
  ...
  goToViews(): void {
    if (this.hasUnsavedChanges) {
      if (confirm('There are unsaved changes. Leave page without saving?')) {
        this.router.navigate(['settings/building-blocks'], {relativeTo: this.route.parent});
      }
    }
