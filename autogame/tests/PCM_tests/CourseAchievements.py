...
QUIZGRADESURL="http://groups.ist.utl.pt/pcm/moodlequizgrades.php?c=8"
PEERRATINGSURL="http://groups.ist.utl.pt/pcm/moodlePeerVotes.php?c=8"
#LOGFILEURL="http://groups.ist.utl.pt/pcm/moodleLogs.php?t=1396264388&c=4"
LOGFILEURL="http://groups.ist.utl.pt/pcm/moodleLogs.php?c=8"
METADIR="meta"
LOCALMETADIR = "../course/metadata/1920/gamification"
...
METADATA = { "all_lectures_alameda": 24,  #excl. invited lectures
			"all_lectures_tagus": 24,  #excl. invited lectures
			"all_lectures": 21,  #number of slide sets
			"invited_alameda": 2,
			"invited_tagus": 2,
			"all_labs": 11,
			"lab_max_grade": 600,
...
class Award:
	...
	def __str__(self):
		if self.badge:
			return str(self.timestamp)+";"+str(self.student)+";"+self.achievement+";"+str(self.level)+";"+str(self.info)
		else:
			return str(self.timestamp)+";"+str(self.student)+";"+self.achievement+";"+str(self.xp)+";"+str(self.info)
	def __repr__(self):
		return str(self)
		...
		if self.student==other.student and \
			...
			if not self.badge:
				if self.info!=other.info:
					return False
					#((not self.badge and self.info == other.info) or False): #crazy dangerous hack!!!!!!!!!!!!
			if self.xp == other.xp:
...
def FilterLogs(logs, student, action=None, info=None, xp=None):
	if not (type(student)==int or type(student)==str) :
		student=student.num
	res=[]
...
class PCMSpreadsheetParser:
	...
	def _ReadWorksheet(self):
		...
		for f in feed.entry:
			...
			if d["num"]:
				# encoding fix
				# res = res+[ LogLine(d["num"],d["name"].encode("latin1"),time.time(), d["action"], d["xp"], d["info"])]
	...
	##              "PCM\tGabriel Barata\tParticipation\tTheoretical Challenge #4\t2\txpto.html"]:
		if len(l.strip())>0:
			...
			try:
				...
				ignored_lines+=1
				continue
			name=str(name, "utf8")  # because that is what we get from the logs now
...
def read_quiz_grades (students):
	...
	for l in query_results[1:] :
		...
		if len(l.strip()) > 0 :
			...
			studentname = str(studentname, "utf8")								# Because that is what we get from the logs "now" (15/02/2018)
			timestamp = time.mktime(time.strptime(timestamp.strip(), "%d %B %Y, %H:%M %p"))
...
#    PostRatingsLines = read_ratings_logs(students,peer=False)
#    for line in PostRatingsLines:
#        logs[line.num]=logs.get(line.num,[])+[line]
	return logs
...
def read_attendance_exceptions():
    res = {}
    f=codecs.open(os.path.join("meta","distance_quiz.txt"),mode="r")
...
def read_student_list():
	res=[]
	for s in codecs.open(os.path.join(METADIR,"students.txt"),"r"):
...
def read_previous_awards(achievement_list):
	metapath = os.path.dirname(get_path_of_script())
	metapath = os.path.join(metapath,"..","..","course","metadata")
	awards_fpath = os.path.join(metapath,"output","awards.txt")
...
def commit_awards(achieved, achievement_list):
	...
	# print "\n\n\n"
	for s in list(achieved.keys()):   # go through all the students
...
def gen_leaderboard(students, achievement_list, awards):
	...
	f=codecs.open(os.path.join(SITEDIR,"index.html"),"w")
...
#        print a.name
		if a.post_based and student.num in indicators and a.name in indicators[student.num] and len(indicators[student.num][a.name][1])>0:
			if counts=="":
				counts=" [Posts]"
...
def write_overall_XP_list(students,achievement_list, awards):
	f=codecs.open(os.path.join(SITEDIR,"awards_log.html"),"w")
...
#    print "generating page for",student.num
	#print(".")
	f=codecs.open(os.path.join(SITEDIR,str(student.num)+".html"),"w")
...
def gen_achievement_page(achievement, students, achievement_list, awards):
	f=codecs.open(os.path.join(SITEDIR,achievement.name.replace(" ","")+".html"),"w")
...
def gen_tree_page(students, achievement_list, awards, indicators, tree, tree_won, tree_unlocked,logs):
	f=codecs.open(os.path.join(SITEDIR,"tree.html"),"w")
...
def read_tree():
	...
	with codecs.open(os.path.join(LOCALMETADIR,"tree.txt"),"r","utf8") as file:
		for s in csv.reader(file, delimiter=";"):
			if len(s)>0:
...
def save_indicators(indicators):
	...
	f.write("[")
	for i in list(indicators.keys())[:-1]:
	...
	if len(indicators)>0:
		f.write('{ "num": "%s", "indicators": %s }\n' % (list(indicators.keys())[-1], str(indicators[list(indicators.keys())[-1]]).replace("u'","'").replace('u"','"').replace("None","null").replace('False',"False").replace("(","[").replace(")","]").replace(", '",', "').replace("':",'":').replace("{'",'{"').replace("['",'["').replace("',",'",')))
	f.write("]")
	f.close()
...
def export_loglines(lines):
  f = codecs.open("logs/loglines.csv","w")
  f.write("num;name;timestamp;action;xp;info;url\n")
  for s in list(lines.keys()):
      for l in lines[s]:
          try:
...
def run (students, logs, achievements, tree):
	"""
	...
	"""
	achieved, indicators = verify_achievements(logs, students, achievements)
	result = satisfied_tree(students, logs, tree, indicators)
	tree_won, tree_unlocked, tree_achieved, indicators = result
	achieved = MergeAchievements(achieved,tree_achieved)
	awards = commit_awards(achieved, achievements)
	return awards, indicators


def get_path_of_script():
	""" returns the path of this specific script """
	import inspect
	return os.path.abspath(inspect.getfile(inspect.currentframe()))

def read_tree_2 ():
	"""
	Since the 'read_tree' function will not work in every environment because
	of the path to the file, this function was made just for that
	"""
	res=[]
	preconds = []
	path = os.path.dirname(get_path_of_script())
	path = os.path.join(path,"metadata","gamification","tree.txt")
	for s in csv.reader(codecs.open(path,"r","utf8"),delimiter=";"):
		if len(s)>0:
			(level,name,pcs,color,xp) = s
			pcs=pcs.strip()
			pcs = pcs.split("|") #different possible preconditions
