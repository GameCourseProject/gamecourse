...
class Views
{
    ...
    // TODO: check if it stays after refactoring table view type
    ...
    private function parseTableRows(&$rows)
    {
        for ($i = 0; $i < count($rows); ++$i) {
            ...
            if (array_key_exists('style', $row))
                ViewHandler::parseSelf($row['style']);
            if (array_key_exists('class', $row))
                ViewHandler::parseSelf($row['class']);

            ViewHandler::parseVariables($row);
            ViewHandler::parseEvents($row);
            foreach ($row['values'] as &$cell) {
                ViewHandler::parsePart($cell['value']);
            }
            ViewHandler::parseLoop($row);
            ViewHandler::parseVisibilityCondition($row);
        }
    }

    private function processTableRows(&$rows, $viewParams, $visitor)
    {
        ViewHandler::processLoop($rows, $viewParams, $visitor, function (&$row, $viewParams, $visitor) {
            ViewHandler::processVariables($row, $viewParams, $visitor, function ($viewParams, $visitor) use (&$row) {
                if (array_key_exists('style', $row))
                    $row['style'] = $row['style']->accept($visitor)->getValue();
                if (array_key_exists('class', $row))
                    $row['class'] = $row['class']->accept($visitor)->getValue();
                ViewHandler::processEvents($row, $visitor);
                foreach ($row['values'] as &$cell) {
                    ViewHandler::processPart($cell['value'], $viewParams, $visitor);
                }
            });
        });
    }
