...
class Course
{
    ...
    public static function newCourse($courseName, $courseShort, $courseYear, $courseColor, $courseStartDate, $courseEndDate, $courseIsVisible, $courseIsActive, $copyFrom = null)
    {
        ...
        if ($copyFrom !== null) {
            ...
            $newRoleOfOldId = function ($id) use ($newRolesByName, $oldRolesByName) {
                ...
                return $newRolesByName[$oldRolesByName[$id]["name"]]["name"];
            };
        ...
        foreach ($fileData as $course) {
            if (!Core::$systemDB->select("course", ["name" => $course->name, "year" => $course->year])) {
                ...
                foreach ($course->page as $page) {
                    Core::$systemDB->insert("page", ["course" => $courseObj->cid, "name" => $page->name, "roleType" => $page->roleType, "viewId" => $page->viewId, "seqId" => $page->seqId, "isEnabled" => $page->isEnabled]);
                }
                ...
                foreach ($course->template as $template) {
                    ...
                    if ($pageWithView) {
                        ...
                        $newViewId = Core::$systemDB->select('view_template', ['templateId' => $templateId], 'viewId');
                        Core::$systemDB->update('page', ['viewId' => $newViewId], ['viewId' => $pageWithView['viewId'], 'course' => $courseObj->cid]);
                    }
                }
                ...
                if ($replace) {
                    ...
                    foreach ($course->template as $template) {
                        ...
                        $pageWithView = null;
                        if (array_key_exists($aspects[0]['viewId'], $tempPages)) {
                        ...
                        //------ DEAL WITH PAGES -----
                        //if there is a page that contains this template, update viewId to the new viewId that may have changed
                        if ($pageWithView) {
                            $templateId = Core::$systemDB->select("template", ["name" => $template["name"], 'course' => $courseEdit->cid], "id");
    ...
    public function uploadFile(string $file, string $folder, string $filename): string
    {
        // Get folder path relative to course data folder
        $folder = Course::getCourseDataFolder($this->getId(), $this->getName()) . "/" . $folder;

        // Get file extension
        $matches = null;
        preg_match('#^data:\w+/(\w+);base64,#i', $file, $matches);
        $extension = $matches[1];
        ...
        // Upload file
        $file = base64_decode(preg_replace('#^data:\w+/\w+;base64,#i', '', $file));
    ...
    {
        $file = Course::getCourseDataFolder($this->getId(), $this->getName()) . "/" .  $path;
        unlink($file);

        // Delete directory if empty
        $dir = dirname($file);
        if (count(glob($dir . "/*")) === 0) Utils::deleteDirectory($dir);
    }
