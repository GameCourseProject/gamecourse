...
class Notifications extends Module
{
    ...
    const TABLE_PROGRESS_REPORT_HISTORY = self::ID . '_progress_report_history';
    ...
    public function initAPIEndpoints()
    {
        ...
        /**
         * Gets a progress report sent to a specific student.
         *
         * @param int $courseId
         * @param int $userId
         * @param int $seqNr
         */
        API::registerFunction(self::ID, 'getStudentProgressReport', function () {
            API::requireCourseAdminPermission();
            ...
            $seqNr = API::getValue('seqNr');

            list($report) = Notifications::getStudentProgressReport($courseId, $seqNr, $userId);
            API::response(['report' => $report]);
        });
    }
    ...
    /**
     * Gets the progress report HTML template for a specific student,
     ...
     * @return array
     */
    public static function getStudentProgressReport(int $courseId, int $seqNr, int $studentId, $student = null, $info = null): array
    {
        // Get all the information needed if not already given
        if ($info == null) $info = Notifications::getStaticInfo($courseId, $seqNr);
        ...
        foreach ($awardsCurrentPeriod as $award) {
            ...
            $typeDescription = Notifications::awardTypeToDescription($award["type"], $tokensName);
        ...
        $isGeneratingReport = Core::$systemDB->select(Notifications::TABLE_PROGRESS_REPORT, ["course" => $courseId, "seqNr" => $seqNr], "count(*)") == 0;
        $totalXP = $isGeneratingReport ? intval(Core::$systemDB->select(XPLevels::TABLE_XP, ["course" => $courseId, "user" => $studentId], "xp")) :
                                         intval(Core::$systemDB->select(self::TABLE_PROGRESS_REPORT_HISTORY, ["course" => $courseId, "user" => $studentId, "seqNr" => $seqNr], "totalXP"));
    ...
    public static function getStaticInfo(int $courseId, int $seqNr = null): array
    {
        ...
        if ($seqNr != null) { // get previous report
            $reportInfo = Core::$systemDB->select(self::TABLE_PROGRESS_REPORT, ["course" => $courseId, "seqNr" => $seqNr]);
        ...
        $timeLeft = Notifications::datediff(date_create($currentDate), date_create($config["endDate"]), $isWeekly ? "weeks" : "days");
        ...
        $tokensName = Core::$systemDB->select(VirtualCurrency::TABLE_CONFIG, ["course" => $courseId], "name");

        return ['courseName' => $courseName, 'courseYear' => $courseYear, 'profilePageId' => $profilePageId, 'imgsPath' => $imgsPath,
            ...
            'timeLeft' => $timeLeft, 'pieChartURL' => $pieChartURL, 'areaChartURL' => $areaChartURL, 'tokensName' => $tokensName
        ];
    }
