...
//print_r(\GameCourse\DataSchema::getValueWithContinuation('users.user.name', array('users.user' => '73137'), array(), true));

/*$me = \GameCourse\DataSchema::getValueWithContinuation('users.user', array('users.user' => '73967'), array(), true);
echo '<pre>';
print_r($me);
...
$t = microtime(true);
foreach($ids as $id) {
    $cont = \GameCourse\DataSchema::getValueWithContinuation('users.user', array('users.user' => (string)$id), array(), true);
    $visitor = new \GameCourse\Expression\EvaluateVisitor(array('u' => (string)$id, 'user' => $cont));
    for($i = 0; $i < 100; ++$i)
        $o = $v2->accept($visitor);
    var_dump($o);
}
...
/*$cont = \GameCourse\DataSchema::getValueWithContinuation('moduleData.skills.skills.tier', array('moduleData.skills.skills.tier' => 't1'), array('course' => 0), true);
...
/*$cont = \GameCourse\DataSchema::getValueWithContinuation('course.users.user', array('course.users.user' => '73137'), array('course' => 0), true);
//$cont = $cont->execute('skills.skill', array('skills.skill' => 2));
echo '<pre>';
print_r($cont);
echo '</pre>';
...
/*$visitor = new \GameCourse\Expression\EvaluateVisitor(array('course' => 0));
